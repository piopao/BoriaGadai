...
GameDiscription gd = db.getGameDiscription(request.getParameter("gameName"));
...
out.println("<a id= \"gameButt\" href= \""+ gd.getGameLink() +"\" class=\"btn btn-success\" role=\"button\">ვიმკითხაოთ</a>");
...
for(int i=0; i<revArr.size(); i++){
	...
	printing+= "<div class=name ><a href="+ "\"Temp.html?userId=" + rev.getUserId() +"\">" + rev.getUserFullName() + "</a></div> ";
	int stars = rev.getStars();
	printing += "<div class = stars> ";
	for(int k=0; k<stars; k++){
		printing+= "<img src=\"./Images/Stars/star-full.jpg\" > ";
	}	
	for(int z = 0; z<5 - stars; z++){
		printing+= "<img src=\"./Images/Stars/star-empty.jpg\" > ";	}	
	printing+= " </div>"; //stars daixura
	printing+= "<h5 class = date> " + rev.getDate() + "</h5>"; 	
	printing+= " </div> "; //box daixura	
	printing+= "<div class = textbox> <p class = text>" + rev.getReviewText() + "</p>	 </div>";	
